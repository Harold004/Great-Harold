...
<option <?php echo ($periodeFiltre=="N30"   ?" selected":""); ?> value="N30">Les 30 dernières</option>
	...
	foreach ($listeAnneesMois as $anneeMois) {
		list($annee, $mois) = explode("-", $anneeMois);
		$mois=intval($mois);
...
<?php
	foreach ($sorties as $sortie) {
		...
		echo "<tr style=\"cursor:pointer;\" onclick=\"window.location='consultersSortie.php?id=$sortie->idSortie'\"><td title=\"$ressources\"><b>$sortie->nom</b></td><td style=\"cursor:pointer;\" onclick=\"window.location='consulterSortie.php?id=$sortie->idSortie'\">$commentaire</td><td>$sortie->date</td><td style=\"color:$couleurEtat;\">$libelleEtat</td><td class=\"tdPrix\">$sortie->coutTTCTotal</td><td class=\"tdQuantite\">$sortie->nbreArticles</td>";
...
<br><br>
<a class="menu" href="ajoutersSortie.php">Ajouter une sortie</a><br>
<a class="menu" href="consultersSortiesCorbeille.php">Voir la corbeille</a><br>
<a class="menu" href="receptionniste.php">*** Retour Page Accueil ***</a>
