...
<br>
<form method="POST" name="leForm" action="editersStock_trt.php">
...
<?php
	...
	foreach ($stock->tLigneStock as $ligneStock) {
		...
		if (!empty($keyword) && strpos(strtolower($nomArticle), strtolower($keyword)) === false) {
			continue; // Ignorer cet article et passer à l'itération suivante
		}
...
&nbsp;&nbsp;&nbsp
<button type="button" class="boutonAnnuler" onclick="javascript:window.location='visualisersStock.php'">Annuler</button>
